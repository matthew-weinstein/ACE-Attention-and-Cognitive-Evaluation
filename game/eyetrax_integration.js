/**
 * Eye Tracker Integration Module
 *
 * This module handles communication between the Electron app and the Python
 * eye tracking script. It spawns the Python process and manages message passing.
 */
...
class EyeTraxTracker {
  ...
  initialize() {
    return new Promise((resolve, reject) => {
      const scriptPath = path.join(__dirname, "eye_tracker.py");

      console.log("Starting eye tracker process:", scriptPath);
      ...
      this.pythonProcess.stderr.on("data", (data) => {
        ...
        console.log("Eye tracker stderr:", message);
      ...
      setTimeout(() => {
        if (!this.isRunning) {
          console.error("Eye tracker initialization timeout");
          reject(new Error("Eye tracker initialization timeout - make sure Python and eyetrax are installed"));
        }
      }, 15000);
    });
  }
  ...
  handleMessage(message) {
    ...
    switch (message.type) {
      ...
      case "calibration":
        if (message.action === "started" && this.callbacks.onCalibrationStarted) {
          this.callbacks.onCalibrationStarted(message.data);
        } else if (message.action === "instruction" && this.callbacks.onCalibrationInstruction) {
          this.callbacks.onCalibrationInstruction(message.data);
        } else if (message.action === "completed" && this.callbacks.onCalibrationCompleted) {
  ...
  sendCommand(action, data = {}) {
    if (!this.pythonProcess) {
      console.error("Eye tracker Python process not initialized");
      return false;
    }

    if (!this.isRunning) {
      console.error("Eye tracker Python process not running (not ready yet)");
      return false;
    }
    ...
    try {
      console.log("Sending command to eye tracker:", command);
      this.pythonProcess.stdin.write(JSON.stringify(command) + "\n");
      return true;
    } catch (error) {
      console.error("Failed to send command to eye tracker:", error);
      return false;
    }
  }
   ...
   */
  shutdown() {
    console.log("Shutting down eye tracker...");
    ...
    if (this.pythonProcess) {
      ...
      setTimeout(() => {
        if (this.pythonProcess) {
          console.log("Force killing eye tracker process");
          this.pythonProcess.kill('SIGTERM');
          
          setTimeout(() => {
            if (this.pythonProcess) {
              console.log("Force killing eye tracker process (SIGKILL)");
